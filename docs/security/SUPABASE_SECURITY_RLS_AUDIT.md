...
| SEC-004 | P1 | CLOSED WITH AUDITED AUTH ALLOWLIST | `20260911102447_secure_security_definer_execute_surface.sql` revokes anon execution on all public `SECURITY DEFINER` functions and allowlists intended authenticated RPCs. `20260914082527_close_pm008_security_definer_allowlist.sql` makes legacy `perform_refill(uuid)` service-role only. | Branch advisor no longer reports `anon_security_definer_function_executable`; catalog confirms refill/dispense/helper grants are narrowed; `security_rls_regression.sql` now enforces the authenticated SECURITY DEFINER allowlist and refill auth cases. |
...
| SEC-008 | P2 / Functional blocker | FIXED ON BRANCH | `20260911102454_admin_functional_rls_and_push_tokens.sql` adds org-scoped admin policies for machine config and coverage tables; `20260911110015_tighten_machine_table_grants.sql` closes residual machine write grants; `20260914100050_fix_admin_coverage_suggested_assignment_delete.sql` restores the admin coverage regenerate flow by granting DELETE with RLS restricted to same-org `suggested` rows; `20260914100936_fix_machine_consumables_fill_percent_trigger.sql` lets tank edits update derived fill percent through a locked-down trigger helper. | Catalog confirms admin policies exist, direct machine insert/delete is revoked, only `temperature_mode`/`updated_at` are column-updatable, and the RLS regression covers admin deletion of suggested coverage assignments plus admin tank config save. |
| SEC-009 | P2 / Functional blocker | FIXED ON BRANCH | `20260911102440_lock_profile_and_ticket_mutations.sql` adds trigger guards for ticket resource/tenant mutation; `20260914132346_fix_ticket_protected_columns_trigger_definer.sql` lets that trigger call the locked-down scope helper without exposing the helper to app roles. | Trigger and helper installed; `tickets` delete is revoked; update remains constrained by policy plus protected-column trigger; RLS regression covers admin ticket reopen. |
