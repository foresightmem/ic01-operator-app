-- Incremental deploy for customer/site/machine onboarding on ic01-dev.
-- This migration intentionally does not recreate the historical core schema.
...
revoke all on function public.client_has_operator_access(uuid, uuid) from public, anon;
grant execute on function public.client_has_operator_access(uuid, uuid) to authenticated;
...
revoke all on function public.site_has_operator_access(uuid, uuid) from public, anon;
grant execute on function public.site_has_operator_access(uuid, uuid) to authenticated;

revoke all on function public.machine_has_operator_access(uuid, uuid) from public, anon;
grant execute on function public.machine_has_operator_access(uuid, uuid) to authenticated;
...
begin
  ...
  insert into public.sites (client_id, name, address, created_by, organization_id)
  values (p_client_id, coalesce(v_site_name, 'Sede'), v_site_address, v_uid, v_organization_id)
  returning id into v_site_id;
...
revoke all on function public.create_client_with_primary_site(text, text, text) from public, anon;
grant execute on function public.create_client_with_primary_site(text, text, text) to authenticated;

revoke all on function public.add_site_to_client(uuid, text, text) from public, anon;
grant execute on function public.add_site_to_client(uuid, text, text) to authenticated;

revoke all on function public.create_machine_for_site(uuid, uuid, text, text, integer, uuid, text) from public, anon;
grant execute on function public.create_machine_for_site(uuid, uuid, text, text, integer, uuid, text) to authenticated;
...
drop policy if exists sites_select_created_or_client_created_by_user on public.sites;
...
drop policy if exists machine_consumables_select_by_machine_access on public.machine_consumables;
