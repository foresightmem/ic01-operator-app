...
alter table public.sites
  add constraint sites_latitude_range
    check (latitude is null or latitude between -90 and 90),
  add constraint sites_longitude_range
...
create or replace function public.create_client_with_primary_site(
  ...
  p_site_name text,
  p_site_city text,
  p_site_latitude double precision,
  p_site_longitude double precision
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_site_latitude is not null and (p_site_latitude < -90 or p_site_latitude > 90) then
    raise exception 'Latitudine sede non valida';
  end if;

  if p_site_longitude is not null and (p_site_longitude < -180 or p_site_longitude > 180) then
    raise exception 'Longitudine sede non valida';
  end if;
  ...
  return jsonb_build_object(
    ...
    'site_latitude', p_site_latitude,
    'site_longitude', p_site_longitude,
    'role', v_role,
...
revoke all on function public.create_client_with_primary_site(
  text,
  text,
  text,
  text,
...
grant execute on function public.create_client_with_primary_site(
  text,
  text,
  text,
  text,
...
revoke all on function public.add_site_to_client(
  uuid,
  text,
  text,
  text,
...
grant execute on function public.add_site_to_client(
  uuid,
  text,
  text,
  text,
...
comment on column public.sites.latitude
  is 'Latitude resolved from the site address, when available.';

comment on column public.sites.longitude
  is 'Longitude resolved from the site address, when available.';
...
comment on function public.create_client_with_primary_site(
  text,
  text,
  text,
  text,
  double precision,
  double precision
) is 'Creates a client and first site atomically, including parsed city and coordinates when provided.';
...
comment on function public.add_site_to_client(
  uuid,
  text,
  text,
  text,
  double precision,
  double precision
) is 'Adds a site to an existing client, including parsed city and coordinates when provided.';
